...
Fetches and stores YouTube channel metadata in Turso/SQLite fo#!/usr/bin/env python3
"""
YouTube Channel Metadata Fetcher
...
def fetch_channel_data(
    ...
    try:
        ...
        # ================================================================
        # TRANSCRIPTS - Only for videos without transcripts
        # ================================================================
        if fetch_transcripts:
            videos_needing_transcripts = get_videos_without_transcripts(conn, channel_id)
            
            if videos_needing_transcripts:
                log.info(f"Fetching transcripts for {len(videos_needing_transcripts)} videos")
...
def dry_run_channel(
    ...
    # Check existing data
    existing_video_ids = get_existing_video_ids(conn, channel_id)
    videos_without_transcripts = get_videos_without_transcripts(conn, channel_id) if fetch_transcripts else []
    ...
    log.info(f"  Videos without transcripts: {len(videos_without_transcripts)}")
